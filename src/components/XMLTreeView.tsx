...
const TreeNode: React.FC<TreeNodeProps> = ({ 
  ...
  if (isObject(data)) {
    return (
      <div className="ml-2 sm:ml-4">
        <div className={`flex items-center space-x-2 py-2 px-2 sm:px-3 rounded-lg transition-all duration-200
          ...
          <button
            ...
            className="flex-1 min-w-0 flex items-center space-x-2 sm:space-x-3 text-left group"
          >
            ...
            <span className="font-medium text-sm sm:text-base truncate text-gray-700 dark:text-gray-200 
              group-hover:text-blue-600 dark:group-hover:text-blue-400">
              {name}
            </span>
            <span className="flex-shrink-0 px-2 py-0.5 text-xs rounded-full bg-gray-100 dark:bg-gray-700 
        ...
        {isExpanded && (
          <div className="ml-4 sm:ml-6 mt-1 border-l-2 border-gray-200 dark:border-gray-700">
  ...
  return (
    <div className={`ml-2 sm:ml-4 flex items-center space-x-2 sm:space-x-3 py-2 px-2 sm:px-3 
      ...
      <div className="flex items-center space-x-2 min-w-0 flex-1">
        <span className="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">{name}:</span>
        <span className="text-sm text-gray-900 dark:text-gray-100 bg-gray-100 dark:bg-gray-700 
          px-2 py-0.5 rounded truncate">{String(data)}</span>
      </div>
    </div>
  );
};
...
export default function XMLTreeView({ data, onSelect, selectedPaths }: XMLTreeViewProps) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 
      dark:border-gray-700 p-2 sm:p-4">
      {Object.entries(data).map(([key, value]) => (
        <TreeNode
          key={key}
          data={value}
